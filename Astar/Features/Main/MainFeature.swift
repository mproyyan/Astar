        ...
        Reduce { state, action in
            ...
            case let .map(.delegate(.walkerStatusChanged(id, newStatus))):
                if let idx = state.people.firstIndex(where: { $0.id == id }) {
                    ...
                    let cloudKitUserId = person.cloudKitUserId ?? person.appleUserId
                    return .run { _ in
                        try? await usersClient.updateUserStatus(cloudKitUserId ?? "", newStatus)
                    }
                }
                return .none
                
            case let .map(.updateTrackingLocation(coordinate)):
                guard let userProfile = state.login.userProfile else { return .none }
                return .run { _ in
                    let coordArray = [coordinate.latitude, coordinate.longitude]
                    guard let encodedData = try? JSONEncoder().encode(coordArray) else { return }
                    
                    _ = try? await trackingClient.pushLocationPing(
                        userProfile.cloudKitUserId ?? userProfile.appleUserId, encodedData
                    )
                }
            ...
            case let .map(.delegate(.companionStatusChanged(newStatus))):
                ...
                guard let currentUserId = state.login.userProfile?.cloudKitUserId ?? state.login.userProfile?.appleUserId else {
                    return .none
                }
                
                return .run { _ in
                    try? await usersClient.updateUserStatus(currentUserId, newStatus)
                }
