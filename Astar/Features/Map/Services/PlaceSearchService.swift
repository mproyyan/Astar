...
enum PlaceSearchService {
    ...
    static func searchPlaces(
        ...
        // 2. Fetch completions from MKLocalSearchCompleter for comprehensive street/address matches
        let completer = SearchCompleterActor()
        let completions = await completer.getCompletions(query: cleanQuery, region: searchRegion)

        // Resolve top completions concurrently
        await withTaskGroup(of: SavedPlace?.self) { group in
            for completion in completions.prefix(12) {
                group.addTask { @MainActor in
                    ...
                    // Fallback for completion if geocoding takes too long
        ...
        // 4. Rank all results from most relevant (top) to least relevant (bottom)
    ...
    private static func rankResults(
        ...
        let scored = places.map { place -> ScoredPlace in
            ...
            if lowerName == lowerQuery {
                ...
                score += 100  // Address match
            } else {
                score += 10   // General POI match
            }

            // 2. Proximity bonus (closer places get a boost within the same match tier)
    ...
    private static func fallbackSearch(query: String, userLocation: CLLocationCoordinate2D?) -> [SavedPlace] {
        let matched = MapSampleData.allSearchablePlaces.filter {
...
@MainActor
private final class SearchCompleterActor: NSObject, MKLocalSearchCompleterDelegate {
